...
void SystemData::triggerPSP()
{
	...
	if (!GetCommState(hSerial, &dcbSerialParams)) {
		std::cout << "Error getting COM state\n";
		CloseHandle(hSerial);
		return;
	}
	...
	if (!SetCommState(hSerial, &dcbSerialParams)) {
		std::cout << "Error setting COM state\n";
		CloseHandle(hSerial);
		return;
	}

	const char* data = "0:WT+STAT=1\r\n";//"WT+STAT=1\r\n";//"0:WT+STAT=1\r\n";
	DWORD bytesWritten;

	WriteFile(hSerial, data, strlen(data), &bytesWritten, nullptr);

	std::cout << "Sent WAA (" << bytesWritten << " bytes)\n";
