...
void VisionApp::createSubRecipe()
{
	//Note: both main and subrecipe can direct save same variables (view, vo, etc) during creation
	// no need to clear and recreate defaults

	//Create subrecipe folder
	Common::Directory::CurrentRecipe = Common::Directory::CurrentRecipe + "/subrecipe1"; //number for future expansion
	Common::Directory::setupRecipeDir(Common::Directory::CurrentRecipe);
...
void VisionApp::switchSubRecipe(const QString& subrecipe)
{
	TimeLogger timer;
	
	//save current subrecipe before switching
	saveRecipe();
	...
	Common::Directory::CurrentRecipe = subrecipe;
	Common::Directory::CurrentImageSetPath = Common::Directory::getRecipeSetupImagePath();
	...
	_optics3DTab->loadOptics3DToUi();
	_optics3DTab->refreshCurrentOptics3DFromJson();
	_optics3DTab->syncToRecipeOptics3D();
	...
	if (ui.stackedWidgetViewSelection->currentIndex() != 6) toggleWorldView();
...
void VisionApp::switchToMainRecipe()
{
	...
	ct::logger::info("Switching to MAIN recipe");

	SystemData::instance()._switchingRecipe = true;

	QSignalBlocker sb(ui.toolButton_switchToMainRecipe);
	QSignalBlocker sb1(ui.toolButton_switchToSubRecipe);
	ui.toolButton_switchToMainRecipe->setChecked(true);
	ui.toolButton_switchToSubRecipe->setChecked(false);

	auto mainRecipe = Common::Directory::CurrentRecipe;
	mainRecipe.remove("/subrecipe1");
	switchSubRecipe(mainRecipe);

	SystemData::instance()._switchingRecipe = false;
	ct::logger::info("Done switching to MAIN recipe");
}

void VisionApp::switchToSubRecipe()
{
	//if (ui.toolButton_switchToSubRecipe->isChecked()) return;
	if (Common::Directory::CurrentRecipe.contains("subrecipe1"))
	{
		return;
	}

	ct::logger::info("Switching to SUB recipe");

	SystemData::instance()._switchingRecipe = true;

	QSignalBlocker sb(ui.toolButton_switchToMainRecipe);
	QSignalBlocker sb1(ui.toolButton_switchToSubRecipe);
	ui.toolButton_switchToMainRecipe->setChecked(false);
	ui.toolButton_switchToSubRecipe->setChecked(true);

	switchSubRecipe(Common::Directory::CurrentRecipe + "/subrecipe1");

	SystemData::instance()._switchingRecipe = false;
	ct::logger::info("Done switching to SUB recipe");
}
